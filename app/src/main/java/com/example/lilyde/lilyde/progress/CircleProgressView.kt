...
import com.example.lilyde.util.Utils.dp2px
import com.example.lilyde.util.Utils.sp2px
...
class CircleProgressView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ProgressBar(context, attrs, defStyleAttr) {
    ...
    private fun drawFillInArcCircle(canvas: Canvas) {
        ...
        // 绘制外层圆环
        rectF?.let { mOutPaint?.let { it1 -> canvas.drawArc(it, 0f, 360f, false, it1) } }
        // 绘制内层进度实心圆弧
        // 内层圆弧半径
        val reachArc = progress * 1.0f / max * 360
        rectInner?.let { mReachPaint?.let { it1 -> canvas.drawArc(it, mStartArc.toFloat(), reachArc, true, it1) } }

        // 绘制未到达进度
        if (reachArc != 360f) {
            rectInner?.let { mNormalPaint?.let { it1 -> canvas.drawArc(it, reachArc + mStartArc, 360 - reachArc, true, it1) } }
        }
        canvas.restore()
    }
    ...
    private fun drawNormalCircle(canvas: Canvas) {
        ...
        // 绘制未到达进度
        if (reachArc != 360f) {
            rectF?.let { mNormalPaint?.let { it1 -> canvas.drawArc(it, reachArc + mStartArc, 360 - reachArc, false, it1) } }
        }
        // 绘制已到达进度
        mReachPaint?.let { rectF?.let { it1 -> canvas.drawArc(it1, mStartArc.toFloat(), reachArc, false, it) } }
        canvas.restore()
    }
