...
import com.example.lilyde.config.GlideConfigImpl
import com.example.lilyde.progress.*
import com.example.lilyde.transformation.*
import java.io.File
...
object Lilyde {
    ...
    var diskCachePool: DiskCachePool = DiskCachePool()
     ...
     * 缓存
     */
    fun putImageToDiskCache(folderName:String, cacheName: String,url: String?){
        val diskLruCacheFactory = DiskLruCacheFactory(folderName,1000)
        diskCachePool.add(diskLruCacheFactory,cacheName)
...
//        LilydeApp.get(context).requestManagerRetriever[context].clear(imageView!!)
    }
    ...
    fun downloadImageToGallery(context: Context, imgUrl: String?) {
        ...
        Observable.create(ObservableOnSubscribe { emitter: ObservableEmitter<File?> ->
            ...
            val fileParentPath = Environment.getExternalStorageDirectory().absolutePath + "/Lilyde/Image"
            ...
            //保存的文件名
            val fileName = "Lilyde_" + System.currentTimeMillis() + "." + extension
            //目标文件
